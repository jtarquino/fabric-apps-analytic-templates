...
describe("useSemanticModelQuery", () => {
    ...
    it("ignores a stale result after the query changes", async () => {
        ...
        const latestResult = {
            status: "success" as const,
            table: { columns: [{ name: "Latest" }], rows: [[2]] },
            fromCache: false,
            cachedAt: undefined,
        };
        mockQuery.mockReturnValueOnce(first).mockResolvedValueOnce(latestResult);

        const { result, rerender } = renderHook(
            ({ query }) => useSemanticModelQuery({ connection: "model", query }),
            { initialProps: { query: "EVALUATE ROW(\"Value\", 1)" } },
        );

        rerender({ query: "EVALUATE ROW(\"Value\", 2)" });
        await waitFor(() => expect(result.current.data).toEqual(latestResult));

        resolveFirst?.({
            status: "success",
            table: { columns: [{ name: "Stale" }], rows: [[1]] },
            fromCache: false,
            cachedAt: undefined,
        });
        await act(async () => {
        ...
        expect(result.current.data).toEqual(latestResult);
    });
});
